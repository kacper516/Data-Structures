...
void f_show_adjacency_matrix(int **array2d, float max)
{
	for (int i = 0; i < max; i++)
	{
		for (int j = 0; j < max; j++)
		{
			if (array2d[i][j] == 1 or array2d[i][j] == -1)
				std::cout << 1;
			else
...
int **f_cr_array2d(float max, float edges)
{
	try
	{
		int row = edges, col = max;
		int **array2d = new int *[int(edges)];
		for (int i = 0; i < row; i++)
		{
...
void f_neighbourhood(int **array_neigbourhood, int **array2d, float edges, float max)
{
	...
	for (int i = 0; i < max; i++)
	{
		//+1 because verts starts with 1 not 0
