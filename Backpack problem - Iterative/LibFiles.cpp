...
void f_save_to_class(ClassItem *items, std::string file_name)
{
	std::ifstream file;
	std::string line;
	f_open_file_read(file_name, file);

	// read file was in the main, so we set the reading again, skip 1 line because there is infromation about weight
	std::getline(file, line);
	int iterator = 1;
	while (!file.eof())
	{
		file >> items[iterator].item_name;
		file >> items[iterator].item_weight;
		file >> items[iterator].item_price;
		iterator++;
	}
	file.close();
}
